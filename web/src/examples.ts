...
// `skipInLoop`: left out of `?autoplay` (see autoplay.ts) - a contrast for a live
// walkthrough, but a dead beat in a recording, which should end on the dashboard.
export const EXAMPLES: { label: string; sql: string; skipInLoop?: boolean }[] = [
  ...
  { label: "Plain SQL (no clause)", sql: "SELECT * FROM products LIMIT 20", skipInLoop: true },
];
